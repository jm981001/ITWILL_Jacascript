...
// 		alert(id);
	...
	function check() {
		...
		//submit 버튼과 도잉ㄹ한 동작을 자바스크립트 내에서 수행하려면
		//document.폼name속성명.submit()
		document.fr.submit();
	}
	...
	function check2(){
		...
		if(document.fr.id.value == "") {
			...
			document.fr.id.focus(); // 아이디 입력창에 커서(포커스) 요청
			// 현재 수행중인 함수(check()) 를 종료하고 빠져나감
			return  false; //현재 함수에서 submit동작을 취소하기위해 false값 리턴
		}else if(document.fr.passwd.value.length==0){
			alert("비밀번호 입력 필수!");
			document.fr.passwd.focus();
			return  false;
		...
		return true; //체크를 완료하고 submit을 수행하려면 true 값 리턴
	}
